...
switch Cfg.responseDevice
   ...
   case 'LUMINA'
        enter_button_index = 2;  
        left_button_index  = 2;
        right_button_index = 1;
    case 'KEYBOARD'  
        enter_button_index = 32;   % space key
        left_button_index  = 37;
        right_button_index = 39;   
end
...
while remaining_time > 0
    [x, y, buttons, t0, t1] = ASF_waitForResponse(Cfg, remaining_time);
    if any(buttons) && (t1 - timeLastResp > 0.05)
        if buttons(left_button_index) > 0 || buttons(right_button_index)
            if first_button_press
                rating.value = 5;
                first_button_press = false;
            elseif buttons(left_button_index) > 0
                rating.value = rem(rating.value,9) - 1;
                if rating.value < 1
                    rating.value = 9 + rating.value;
                end
            else
                rating.value = rem(rating.value,9) + 1;
            end
